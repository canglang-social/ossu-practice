...
def pairwise_div(Lnum, Ldenom):
    ...
    # m1
    assert len(Lnum) != 0, "Lnum is empty"
    assert len(Ldenom) != 0, "Ldenom is empty"
    assert len(Lnum) == len(Ldenom), "Lnum and Ldenom is not equal length"

    my_list = []
    for i in range(len(Lnum)):
        try:
            my_list.append(Lnum[i] / Ldenom[i])
        except ZeroDivisionError:
            raise ValueError
    return my_list
...
# print(1,get_stats(test_grades, avg1))
...
print(2, get_stats(test_grades, avg2))
...
print(3, get_stats(test_grades, avg3))
...
print(4, get_stats(test_grades, avg4))
...
def pairwise_div(Lnum, Ldenom):
    ...
    # your code here
    assert len(Lnum) == len(Ldenom), "not equal"
    assert len(Lnum) != 0, "empty list"
    my_l = []
    for i in len(Lnum):
        try:
            my_l.append(Lnum[i] / Ldenom[i])
        except:
            raise ValueError
    return my_l
