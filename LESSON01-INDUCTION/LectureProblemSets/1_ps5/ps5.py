...
def filter(pixels_list, color):
    ...
    for i in pixels_list:
        r = matrix_multiply(make_matrix(color), i)
        if type(r) is float:
            new_list.append(int(r))
        else:
            new_list.append(tuple([int(j) for j in r]))
    return new_list
...
def extract_end_bits(num_end_bits, pixel):
    ...
    return (
        ...
        else tuple([i % (2**num_end_bits) for i in pixel])
    )
...
def reveal_bw_image(filename):
    ...
    new_pix_list = []
    for p in pix_list:
        end_bits = extract_end_bits(1, p)
        # need to rescale the pixel values, so they can make use of the full range of available values
        rescale_bits = end_bits * (255 // 2)
        new_pix_list.append(rescale_bits)
    img = pix_to_img(new_pix_list, Image.open(filename).size, "L")
    return img
...
def reveal_color_image(filename):
    ...
    for p in pix_list:
        new_pix = []
        for i in p:
            end_bits = extract_end_bits(3, i)
            rescale_bits = end_bits * (255 // 2**3)
            new_pix.append(rescale_bits)
        new_pix_list.append(tuple(new_pix))
    img = pix_to_img(new_pix_list, Image.open(filename).size, "RGB")
    return img
...
def draw_kerb(filename, kerb):
    ...
    """
    im = Image.open(filename)
    font = ImageFont.truetype("LectureProblemSets/1_ps5/noto-sans-mono.ttf", 40)
...
def main():
    ...
    im = Image.open("LectureProblemSets/1_ps5/image_15.png")
    width, height = im.size
    pixels = img_to_pix("LectureProblemSets/1_ps5/image_15.png")
    ...
    im2.save("LectureProblemSets/1_ps5/not_red_image_15.bmp")
    im2.show()

    # Uncomment the following lines to test part 2
    im = reveal_image("LectureProblemSets/1_ps5/hidden1.bmp")
    im.show()
    # im.save("LectureProblemSets/1_ps5/not_hidden1.bmp")

    im2 = reveal_image("LectureProblemSets/1_ps5/hidden2.bmp")
    im2.show()
    # im2.save("LectureProblemSets/1_ps5/not_hidden2.bmp")
    for i in [
        "LectureProblemSets/1_ps5/not_red_image_15.bmp",
        "LectureProblemSets/1_ps5/not_hidden1.bmp",
        "LectureProblemSets/1_ps5/not_hidden2.bmp",
    ]:
        draw_kerb(i, "VegeKiwi")
