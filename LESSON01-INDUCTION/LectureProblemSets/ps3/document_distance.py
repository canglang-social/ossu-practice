...
# Name: FelixOrion
# Collaborators:
# Time: 251212 ThirdClass ForthClass
...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    for k1 in freq_dict1:
        if k1 in freq_dict2:
            diff += abs(freq_dict1[k1] - freq_dict2[k1])
            sum += freq_dict1[k1] + freq_dict2[k1]
        else:
            diff += abs(freq_dict1[k1] - 0)
            sum += freq_dict1[k1]
    for k2 in freq_dict2:
        if k2 not in freq_dict1:
            diff += abs(0 - freq_dict2[k2])
            sum += freq_dict2[k2]
    return round(1 - diff / sum, 2)
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    ...
    tf_dict = {}
    for w in freq_dict:
        tf_dict[w] = freq_dict[w] / len(word_list)
    return tf_dict
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    idf_dict = {}
    for w in df_dict:
        idf_dict[w] = math.log10(len(file_paths) / df_dict[w])
    return idf_dict
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    * TF-IDF(i) = TF(i) * IDF(i)
    ...
    for w in tf_dict:
        tf_idf_list.append((w, tf_dict[w] * idf_dict[w]))
    return sorted(tf_idf_list, key=lambda item: (item[1], item[0]))
...
if __name__ == "__main__":
    ...
    test_directory = "LectureProblemSets/ps3/tests/student_tests/"
    ...
    tf_text_file = "LectureProblemSets/ps3/tests/student_tests/hello_world.txt"
    idf_text_files = [
