...
class Message(object):
    ...
    def shift_char(self, char, shift):
        ...
        old_num = ord(char)
        if shift >= 0:
            new_num = old_num + shift % 95
            if new_num > 126:
                new_num = 31 + new_num - 126
        else:
            new_num = old_num + shift % (-95)
            if new_num < 32:
                new_num = 127 - 32 + new_num
        return chr(new_num)
    ...
    def apply_pad(self, pad):
        ...
        """
        assert len(pad) == len(self.get_text()), "length is not equal"
        plaintext = self.get_text()
        ciphertext = ""
        for i in range(len(plaintext)):
            ciphertext += self.shift_char(plaintext[i], pad[i])
        return ciphertext
...
class PlaintextMessage(Message):
    def __init__(self, input_text, pad=None):
        ...
        self.pad = pad[:] if pad else self.generate_pad()
    ...
    def generate_pad(self):
        ...
        for i in self.get_text():
            pad.append(random.randrange(0, 110))
        return pad
...
class EncryptedMessage(Message):
    ...
    def decrypt_message(self, pad):
        ...
        """
        assert len(pad) == len(self.get_text())
        ciphertext = self.get_text()
        plaintext = ""
        for i in range(len(ciphertext)):
            plaintext += self.shift_char(ciphertext[i], -pad[i])
