...
def make_pets(d):
    ...
    # your code here
    for i in d:
        print(f"{i.get_name()} : {d[i].get_name()}")
...
class Employee(Person):
    ...
    def salary_change(self, n):
        ...
        new_s = self.s + n if (self.s + n) >= 0 else 0
        self.s = new_s
        self.s_h.append(new_s)
...
def counts(L):
    ...
    u_p = []
    for i in L:
        if i.get_name() not in u_p:
            u_p.append(i.get_name())
        if type(i) is Person:
            n_p += 1
        elif type(i) is Employee:
            n_e += 1
    return (n_p, n_e, len(u_p))
